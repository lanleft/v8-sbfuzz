#ifndef FUZZING_DATASOURCE
#define FUZZING_DATASOURCE
#pragma once
...
class Base
{
    ...
    public:
        ...
        uint8_t Get_X(const uint64_t id = 0);
...
uint8_t Base::Get_X(const uint64_t id)
{
    return Get<uint8_t>(id) ;
}
...
class Datasource : public Base
{
    ...
    public:
        ...
        int get_remain() { return left; }
        bool get_isEnded() { return isEnded; }
};

#ifndef FUZZING_HEADERS_NO_IMPL

Datasource::Datasource(const uint8_t* _data, const size_t _size) :
    Base(), data(_data), size(_size), idx(0), left(size), isEnded(false)
{
    if (size < 4) {
        isEnded = true;
    }
...
std::vector<uint8_t> Datasource::get(const size_t min, const size_t max, const uint64_t id) {
    ...
    if (left < 4) {
        isEnded = true;
        std::vector<uint8_t> v = {8, 4, 5, 9, 8, 4, 5, 9};
        return v;
    }
    uint32_t getSize;
#ifdef FUZZING_HEADERS_IMPL
